...
file = open("1e6.dat", 'r') # otworzenie pliku w trybie odczytu. Przy próbie zapisu wyskoczyłby błąd)
...
for PESEL in file: # pętla iterująca po pliku linijka po linijce. Nazwa PESEL jest taką "tymczasową" zmienną na potrzeby aktualnego 
    ...
    if not (len(PESEL) == PESEL_length): # sprawdzenie czy PESEL ma poprawną długość. Używamy != bo możemy napotkać zarówno za długie
                                   # jak i za krótkie numery
        invalid_length += 1        # jeśli długość się nie zgadza to zwiększamy zmienną invalid
        continue
    # i przerywamy AKTUALNĄ iterację przechodząc do kolejnej (żeby niepotrzebnie nie wykonywał się dalszy kod)
    ...
    month = int(PESEL[2:4])     # analogicznie, przy czym dodatkowo wykonujemy modulo. Tak, żeby w przypadku innego stulecia niż 1900-1999
    ...
    month = month % 20
    if month > 12 or month == 0: # sprawdzenie czy numer m-ca się zgadza. Jeśli nie, zwiększamy zmienną, przerywamy aktualną iterację
        invalid_date += 1
        continue
